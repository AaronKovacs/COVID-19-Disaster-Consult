...
(function ( $ ) {
    ...
    $.fn.readMore = function( options ) {
        ...
        return this.each(function() {
            // Get elements
            var $container = $(this);
            var $smallHeight = $container.height();
            var $expand = $container.find(settings.expandTrigger);
            var $collapser = $container.find(settings.collapserTrigger);

            // Get the height of the container
            var totalHeight = $container[0].scrollHeight;
            var previousHeight = $container[0].scrollHeight;
            ...
            $expand.on('click', function() {
                ...
                $container
                    .css({ "height": $container.height() +50, "max-height": 99999 })
                    .animate({ "height": totalHeight });

                $expand.fadeOut(); // fade out prompt
                $collapser.fadeIn(); // fade in collapser
            ...
            $collapser.on('click', function() {
                // Set height to prevent instant jump-down when max height is removed
                $container
                  .css('max-height', settings.previewHeight + 'px')
                  .animate({ "height": settings.previewHeight });

                $expand.fadeIn(); // fade in prompt
                $collapser.fadeOut(); // fade out collapser
                return false; // prevent default jump-down action
            });
